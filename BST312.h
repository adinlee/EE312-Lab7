 ...
 *
 * Name: Adin Lee
 * email address: adinlee@utexas.edu
 * UTEID: ael2575
...
{
    if (!isEmpty()){        // do nothing if already empty
        makeEmpty(root);
        root = NULL;
    }
}
...
{
    int cnt = 1;    // since return type is an int
    if (t->left != NULL) {
        cnt += countNodes(t->left);  // will add 1 every recursive call
    }
    if (t->right != NULL) {
        cnt += countNodes(t->right);
    }
    return cnt;
}
